# app.py
#对比上一版本，把“cycle”更改成了“member schedule”,并做了课程表。便于显示。
from flask import Flask, render_template, request, redirect, url_for, session, flash, send_from_directory,current_app, g
...
@app.before_request
def add_timestamp():
    g.timestamp = datetime.now().timestamp()  # 生成当前时间戳并存储在 g 对象中
...
def courses():
    ...
    # 获取总记录数
    count_query = "SELECT COUNT(*) as total FROM Course"
    if request.method == 'POST':
        q = request.form.get('q','').strip()
        if q:
            base_query += " WHERE c.name LIKE %s"
            count_query += " WHERE name LIKE %s"
            cur.execute(count_query, (f'%{q}%',))
        else:
    ...
    if request.method == 'POST' and q:
        cur.execute(base_query, (f'%{q}%',))
    else:
        cur.execute(base_query)
    
    current_page_courses = cur.fetchall()
    
    # 为每个课程添加教练名称
    for course in current_page_courses:
        course['coach_name'] = (course['last_name'] or '') + (course['first_name'] or '')
    
    return render_template('courses.html', 
                         courses=current_page_courses, 
                         q=q, 
                         page=page, 
...
# Course enrollment
# 统一版 Course Enrollment
@app.route('/enroll/<course_id>', methods=['POST', 'GET'])
def enroll(course_id):
    ...
    # 1. 检查是否已经报名
    cur.execute("SELECT * FROM CourseEnrollment WHERE course_id=%s AND member_id=%s", (course_id, user_id))
    if cur.fetchone():
    ...
    cur.execute("SELECT COUNT(*) AS cnt FROM CourseEnrollment WHERE course_id=%s", (course_id,))
    ...
    # 【✅新增】时间冲突检测
    cur.execute("SELECT start_date, start_time FROM Course WHERE course_id = %s", (course_id,))
    course_info = cur.fetchone()

    if course_info:
        weekly = datetime.strptime(str(course_info['start_date']), "%Y-%m-%d").weekday()
        enroll_start_time = course_info['start_time']

        # 查找当前用户已报名的所有课程
        cur.execute("""
            SELECT c.start_date, c.start_time
            FROM CourseEnrollment ce
        ...
        for enrolled in enrolled_courses:
            enrolled_weekly = datetime.strptime(str(enrolled['start_date']), "%Y-%m-%d").weekday()
            enrolled_start_time = enrolled['start_time']

            if enrolled_weekly == weekly and enrolled_start_time == enroll_start_time:
                flash("Time conflict! You already have a course at the same time.")
                return redirect(url_for('courses'))

    # 3. 检查 CourseTemplate 是否存在
    cur.execute("SELECT * FROM CourseTemplate WHERE course_id = %s", (course_id,))
    ...
    if not template:
        ...
        weekly = datetime.strptime(str(c['start_date']), "%Y-%m-%d").weekday() + 1  # 0-6变1-7
        ...
        generate_course_instances(
            cur, template_id, course_id, c['name'], weekly,
            c['start_time'], c['end_time'], c['start_date'], c['end_date'], user_id
        )

    # 4. 插入报名 CourseEnrollment
    cur.execute("""
        INSERT INTO CourseEnrollment (member_id, course_id, enroll_time)
        VALUES (%s, %s, NOW())
    """, (user_id, course_id))
    mysql.connection.commit()
    ...
    if fee_result:
        fee = fee_result['fee']
        cur.execute("""
            INSERT INTO Purchase (member_id, type, ref_id, number, total_fee)
            VALUES (%s, 'course', %s, 1, %s)
        """, (user_id, course_id, fee))
        mysql.connection.commit()

    cur.close()

    flash("Registration successful and cycle generated automatically!")
    return redirect(url_for('member_schedule'))
...
def member_schedule():
    ...
    SELECT ci.course_instance_id,
           ...
           ct.location
    FROM CourseEnrollment ce
    JOIN CourseInstance ci ON ce.course_id = ci.course_id
    ...
    enrolled_instances = cur.fetchall()
    cur.close()

    next_course = enrolled_instances[0] if enrolled_instances else None

    return render_template('course_instances.html', 
                            enrolled_instances=enrolled_instances,
                            next_course=next_course)

# 删除课程模板
@app.route('/delete_course_template/<int:template_id>')
def delete_course_template(template_id):
    if session.get('role') != 'admin':
        flash("Only admin can delete templates.")
        return redirect(url_for('index'))

    cur = mysql.connection.cursor()
    
    # 先删除关联的 CourseInstance（如果有）
    cur.execute("DELETE FROM CourseInstance WHERE template_id = %s", (template_id,))
    
    # 再删除 CourseTemplate 本身
    cur.execute("DELETE FROM CourseTemplate WHERE template_id = %s", (template_id,))
...
def products():
    try:
        ...
        flash("获取产品列表时出错")
        return render_template('products.html', products=[], q='', page=1, total_pages=1)
...
def branch_manage():
    ...
    # 获取总记录数
    cur.execute("SELECT COUNT(*) as total FROM Branch")
    total_records = cur.fetchone()['total']
    total_pages = (total_records + per_page - 1) // per_page
    ...
    return render_template('branch_manage.html', 
                         branches=branches,
                         page=page,
                         total_pages=total_pages)
...
def admin_manage():
    ...
    """)
    staff_total = cur.fetchone()['total']
    staff_total_pages = (staff_total + per_page - 1) // per_page

    # Query staff and admin users with pagination
    cur.execute("""
        ...
        LIMIT %s OFFSET %s
    """, (per_page, (staff_page-1)*per_page))
    ...
    cur.execute("""
        ...
        WHERE p.role = 'member'
    """)
    member_total = cur.fetchone()['total']
    member_total_pages = (member_total + per_page - 1) // per_page

    # Query member users with pagination
    cur.execute("""
        SELECT p.person_id, p.username, p.first_name, p.last_name, p.role, p.phone, b.name AS branch_name
        FROM Person p
        LEFT JOIN Branch b ON p.branch_id = b.branch_id
        ...
        ORDER BY p.person_id
        LIMIT %s OFFSET %s
    """, (per_page, (member_page-1)*per_page))
...
def lost_and_found():
    ...
    offset = (page - 1) * per_page
    cur.execute("""
        SELECT laf.pick_id, laf.pick_name, laf.pick_photo, laf.pick_time, laf.pick_place, 
               laf.description, laf.registrant_username, laf.is_claimed
        FROM LostAndFound laf
        ORDER BY laf.pick_time DESC
        LIMIT %s OFFSET %s
    """, (per_page, offset))
...
def coaches():
    ...
    if request.method == 'POST':
        ...
        if q:
            cur.execute("""
                SELECT p.* 
                FROM Person p 
                WHERE p.role = 'coach' AND (p.first_name LIKE %s OR p.last_name LIKE %s)
                ORDER BY p.person_id
            """, (f'%{q}%', f'%{q}%'))
    ...
    current_page_coaches = coaches[start_idx:end_idx]
    


    # 获取男性教练头像列表
    male_coach_images = []
    male_coach_dir = os.path.join(app.static_folder, 'images/coach_image/male_image')
    if os.path.exists(male_coach_dir):
        male_coach_images = sorted([f for f in os.listdir(male_coach_dir) if f.endswith(('.jpg', '.jpeg', '.png'))])
        male_coach_images = [posixpath.join('images/coach_image/male_image', img) for img in male_coach_images]

    # 获取女性教练头像列表
    female_coach_images = []
    female_coach_dir = os.path.join(app.static_folder, 'images/coach_image/female_image')
    if os.path.exists(female_coach_dir):
        female_coach_images = sorted([f for f in os.listdir(female_coach_dir) if f.endswith(('.jpg', '.jpeg', '.png'))])
        female_coach_images = [posixpath.join('images/coach_image/female_image', img) for img in female_coach_images]
    
    # 为所有教练设置头像和名称
    male_coach_index = 0
    female_coach_index = 0
    npc_index = 0
    
    # 先为所有教练分配头像
    for coach in coaches:
        coach['name'] = f"{coach['last_name']} {coach['first_name']}"
        # 根据性别选择头像
        if coach.get('gender') == 'M':
            if len(male_coach_images) > 0 and male_coach_index < len(male_coach_images):
                coach['image'] = male_coach_images[male_coach_index]
                male_coach_index += 1
            else:
                coach['image'] = 'npc_image/npc_male.jpg'
        elif coach.get('gender') == 'F':
            if len(female_coach_images) > 0 and female_coach_index < len(female_coach_images):
                coach['image'] = female_coach_images[female_coach_index]
                female_coach_index += 1
            else:
                coach['image'] = 'npc_image/npc_female.jpg'
        else:
            # 如果性别未知，使用默认头像
            coach['image'] = 'npc_image/14101745592041_.pic.jpg'
        
        # 如果没有specialty字段，添加默认值
        if 'specialty' not in coach:
            coach['specialty'] = '健身教练'
        # 如果没有description字段，添加默认值
        if 'description' not in coach:
            coach['description'] = ''
    
    return render_template('coaches.html', 
                         coaches=current_page_coaches, 
                         q=q,
                         page=page,
                         total_pages=total_pages)

# 预约教练
@app.route('/book_coach/<coach_id>')
def book_coach(coach_id):
    ...
    # 检查是否已经预约过该教练
    cur.execute("SELECT * FROM CoachBooking WHERE coach_id=%s AND member_id=%s", (coach_id,uid))
    if cur.fetchone():
        flash("您已经预约过该教练")
    ...
    try:
        # 创建预约记录
        cur.execute("""
            INSERT INTO CoachBooking(coach_id, member_id, booking_time) 
            VALUES(%s, %s, NOW())
        """, (coach_id, uid))
        mysql.connection.commit()
        flash("预约成功！")
...
def add_no_cache_headers(response):
    ...
    return response
#新增查看具体教练课程表路由
@app.route('/view_coach_schedule/<int:coach_id>')
def view_coach_schedule(coach_id):
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)

    # 查询教练教授的课程
    cur.execute("""
        SELECT name, start_date, end_date, start_time, end_time, location
        FROM Course
        WHERE coach_id = %s
    """, (coach_id,))
    courses = cur.fetchall()
    ...
    # 查询教练负责的模板（排课计划）
    cur.execute("""
        SELECT name AS course_name, location, weekly, start_time, end_time
        FROM CourseTemplate
        WHERE coach_id = %s
    """, (coach_id,))
    ...
    return render_template('coach_schedule.html', courses=courses, schedule=schedule)
...
@app.route('/gym_chat')
def gym_chat():
    return render_template('gym_chat.html')
