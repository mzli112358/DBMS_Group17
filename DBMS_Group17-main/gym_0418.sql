...
CREATE TABLE CourseTemplate (
    ...
    FOREIGN KEY (coach_id) REFERENCES Person(person_id),
    FOREIGN KEY (branch_id) REFERENCES Branch(branch_id)
    FOREIGN KEY (valid_from) REFERENCES Course(start_date),
    FOREIGN KEY (valid_to) REFERENCES Course(end_date),
    FOREIGN KEY ( max_seats) REFERENCES Course( max_seats),
    FOREIGN KEY (fee) REFERENCES Course(fee),
);

-- 周期课程实例表，新增，课程实例，是一天的单个课程的表
CREATE TABLE CourseInstance (
    course_instance_id VARCHAR(32) PRIMARY KEY,       -- 课程实例编号，如 C101_20240515
    course_name VARCHAR(32)
    template_id INT
    course_id VARCHAR(32),              -- 外键：引用 Course.course_id
    start_datetime DATETIME,
    end_datetime DATETIME,
    status VARCHAR(16) DEFAULT 'present',   --反之则absent
