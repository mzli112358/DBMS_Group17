...
def add_course_template():
    ...
    valid_from = datetime.strptime(form['valid_from'], "%Y-%m-%d").date()
    valid_to = datetime.strptime(form['valid_to'], "%Y-%m-%d").date()
    max_seats = int(form['max_seats'])
    fee = float(form['fee'])
    ...
    cur = mysql.connection.cursor()
    cur.execute("""
        INSERT INTO CourseTemplate (name, coach_id, branch_id, location, weekly, start_time, end_time, valid_from, valid_to, max_seats, fee, requirement)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """, (name, coach_id, branch_id, location, weekly, start_time, end_time, valid_from, valid_to, max_seats, fee, requirement))
    mysql.connection.commit()

    template_id = cur.lastrowid
    generate_course_instances(cur, template_id, course_id, course_name, weekly, start_time, end_time, valid_from, valid_to)

    flash("The template has been successfully created and the course instance has been generated")
...
def course_instances():
    cur = mysql.connection.cursor()
    cur.execute("""
        SELECT ci.*, c.name AS course_name, c.location, c.max_seats, p.first_name, p.last_name
        FROM CourseInstance ci
        JOIN Course c ON ci.course_id = c.course_id
...
#工具函数
def generate_course_instances(cur, template_id, course_id, course_name, weekly, start_time, end_time, valid_from, valid_to):
    ...
    while date <= valid_to:
        if date.weekday() == weekly:
            ...
            cur.execute("""
                INSERT INTO CourseInstance (course_instance_id, course_name, template_id, course_id, start_datetime, end_datetime, status)
                VALUES (%s, %s, %s, %s, %s, %s, 'present')
            """, (course_instance_id, course_name, template_id, course_id, start_dt, end_dt))
